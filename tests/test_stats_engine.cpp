...
protected:
    ...
    PacketData createPacket(const std::string& src_ip, const std::string& dst_ip, 
                           size_t size, const std::string& payload) {
        PacketData pkt;
        pkt.src_ip = src_ip;
        pkt.dst_ip = dst_ip;
        pkt.size = size;
...
TEST_F(StatsEngineDetailedTest, EntropyCalculationAccuracy) {
    ...
    PacketData high_entropy_pkt = createPacket("192.168.1.1", "10.0.0.1", 100, 
        "XmK9pL2qW7vF4nR8tY3uI6oP5aS1dG0zC");
    
    // Test low entropy payload (repetitive)
    PacketData low_entropy_pkt = createPacket("192.168.1.1", "10.0.0.1", 100, 
        std::string(50, 'A'));
    
    default_engine->analyze(high_entropy_pkt);
    double high_entropy = default_engine->get_entropy();
    
    default_engine->analyze(low_entropy_pkt);
...
TEST_F(StatsEngineDetailedTest, EWMASmoothing) {
    ...
    EXPECT_LT(std::abs(rate3 - rate1), std::abs(rate2 - rate1)); // Should move back toward original
}

TEST_F(StatsEngineDetailedTest, DifferentThresholdBehavior) {
    PacketData attack_pkt = createPacket("192.168.1.100", "10.0.0.1", 64, 
        std::string(32, 'X'));  // Low entropy attack
    
    // Build baseline for all engines
    for (int i = 0; i < 5; i++) {
        default_engine->analyze(attack_pkt);
        sensitive_engine->analyze(attack_pkt);
        lenient_engine->analyze(attack_pkt);
    }
...
TEST_F(StatsEngineDetailedTest, StatisticalAnomalyDetection) {
    ...
    for (int i = 0; i < 10; i++) {
        PacketData pkt = createPacket("192.168.1." + std::to_string(i), "10.0.0.1", 
            1400 + (i * 10), "Normal HTTP request data with varying content " + std::to_string(i));
...
TEST_F(StatsEngineDetailedTest, EmptyPayloadHandling) {
    ...
    EXPECT_NO_THROW(default_engine->analyze(empty_pkt));
    
    // Entropy of empty payload should be 0
    default_engine->analyze(empty_pkt);
...
TEST_F(StatsEngineDetailedTest, LargePayloadHandling) {
    ...
    EXPECT_NO_THROW(default_engine->analyze(large_pkt));
...
TEST_F(StatsEngineDetailedTest, EdgeCasePayloads) {
    ...
    for (const auto& payload : edge_payloads) {
        PacketData pkt = createPacket("192.168.1.1", "10.0.0.1", 
            payload.length() + 40, payload);
        
        EXPECT_NO_THROW(default_engine->analyze(pkt));
    }
}
...
TEST_F(StatsEngineDetailedTest, PerformanceUnderLoad) {
    ...
    for (int i = 0; i < 1000; i++) {
        PacketData pkt = createPacket("192.168.1." + std::to_string(i % 100), 
            "10.0.0.1", 1000 + (i % 500), 
            "Packet content " + std::to_string(i));
        default_engine->analyze(pkt);
    }
